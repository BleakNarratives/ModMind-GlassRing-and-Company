...
import { Ghost, Heart, Shield, Sun, Leaf, Cog, Info, Zap, Mic, Activity } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto space-y-16 pb-24 animate-in fade-in duration-1000">
      ...
      {bursting && (
        <div className="fixed inset-0 z-[1000] pointer-events-none flex items-center justify-center">
           <div className={`w-1 h-1 rounded-full animate-[ping_0.8s_ease-out_forwards] ${getHatStyles(activeHat).bg.replace('/10', '')}`} />
           <div className="absolute inset-0 bg-white/5 backdrop-blur-sm animate-out fade-out duration-700" />
        </div>
      )}

      <div className="text-center space-y-6 relative">
        <div className="absolute -top-20 left-1/2 -translate-x-1/2 opacity-10 blur-3xl w-64 h-64 bg-emerald-500 rounded-full animate-pulse" />
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          >
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.5em]">
              {isListening ? 'Vortex Listener: Live' : 'Initiate Voice Cognition'}
            </span>
          </button>
          
          <div className="space-y-2">
            <h2 className="text-7xl font-black tracking-tighter text-white uppercase italic">Cognitive Shards</h2>
            <p className="text-[10px] font-black uppercase tracking-[1em] text-slate-500 translate-x-1">Six Thinking Hats Protocol</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {HATS_DATA.map((hat) => {
          ...
          return (
            ...
            >
              <div className={`w-16 h-16 rounded-[2rem] ${styles.bg} ${styles.color} flex items-center justify-center mb-8 transition-all duration-700 group-hover:rotate-12`}>
                {styles.icon}
              </div>
      ...
      <div className={`etched-glass p-12 rounded-[4.5rem] border-2 transition-all duration-1000 flex flex-col lg:flex-row items-center gap-12 ${getHatStyles(activeHat).border}`}>
        <div className="shrink-0 relative">
          <div className={`w-48 h-48 rounded-full border-2 border-dashed animate-[spin_20s_linear_infinite] opacity-20 ${getHatStyles(activeHat).color.replace('text-', 'border-')}`} />
          <div className="absolute inset-0 flex items-center justify-center">
            <div className={`w-28 h-28 rounded-[2.5rem] glass flex items-center justify-center shadow-2xl scale-110 ${getHatStyles(activeHat).color}`}>
              {getHatStyles(activeHat).icon}
            </div>
          </div>
          <Activity className={`absolute -bottom-4 -right-4 ${getHatStyles(activeHat).color} animate-pulse`} size={32} />
        </div>
        
        <div className="flex-1 space-y-6 text-center lg:text-left">
          <div className="flex flex-col lg:flex-row items-center gap-4">
             <span className={`text-5xl font-black tracking-tighter uppercase italic ${getHatStyles(activeHat).color}`}>Linked: {activeHat}</span>
             <div className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.4em] border transition-all duration-700 ${getHatStyles(activeHat).border} ${getHatStyles(activeHat).color}`}>
               Neural Resonance Verified
             </div>
          </div>
          <p className="text-slate-400 text-xl leading-relaxed font-medium max-w-2xl">
            Sovereign override confirmed. JaneBot's personality matrix is now forced through the <span className={getHatStyles(activeHat).color}>{activeHat} shard</span>. No diplomatic buffers. No filtered data. Just the hat.
          </p>
          <div className="flex gap-4 justify-center lg:justify-start pt-4">
             <div className="h-1 w-20 rounded-full bg-white/5"><div className="h-full w-3/4 bg-current rounded-full" /></div>
             <div className="h-1 w-20 rounded-full bg-white/5"><div className="h-full w-1/2 bg-current rounded-full opacity-50" /></div>
          </div>
        </div>
      </div>
    </div>
